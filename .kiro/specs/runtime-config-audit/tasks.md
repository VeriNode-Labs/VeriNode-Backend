...
- [x] 1. Database migrations (008, 009, 010)
...
- [x] 2. Shared types and error classes (`src/audit/types.ts`)
...
- [x] 3. OTel metrics instruments (`src/audit/metrics.ts`)
...
- [x] 4. HMAC utility function and `AuditLogger` construction guard
...
- [x] 5. `BaselineManager` — serialization and deserialization
...
- [x] 6. `BaselineManager` — capture, getActive, and expireOldBaselines
...
- [x] 7. `AuditLogger` — write with bounded queue and retry
...
- [x] 8. `AuditLogger` — verifyIntegrity, verifyChain, and queryAuditLog
...
- [x] 9. `TokenValidator` extension — `ActorContext` and `requirePermission`
...
- [x] 12. `ConfigAuditService` — orchestrator, start/stop, captureBaseline, rollback, healthCheck
...
- [x] 13. Public module re-export (`src/audit/index.ts`)
...
- [x] 14. Unit tests — `BaselineManager` serialization and capture
...
- [x] 15. Unit tests — `AuditLogger` HMAC, retry, queue eviction, verifyChain
...
- [x] 16. Unit tests — `DriftDetector` (no baseline, critical/non-critical, 5s timeout)
...
- [x] 17. Unit tests — `AlertDispatcher` (redaction, idempotency, async hand-off)
...
- [x] 18. Integration tests — full `ConfigAuditService` event flow
...
- [x] 19. Integration tests — rollback and verifyChain
...
- [x] 20. Monitoring runbook and deployment canary config
